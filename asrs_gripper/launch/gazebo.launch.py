...
def generate_launch_description():
    model_arg = DeclareLaunchArgument(
        name="model", 
        default_value=os.path.join(get_package_share_directory("asrs_gripper"), "urdf", "gripper.xacro"),
        description="Absolute path to the ASRS Arm URDF File"
    )

    env_variable = SetEnvironmentVariable("GAZEBO_MODEL_PATH", os.path.join(get_package_share_directory("gripper"), "share"))
    ...
    spawn_robot = Node(
        ...
        arguments=["-entity", "asrs_gripper", "-topic", "robot_description"]
    )
